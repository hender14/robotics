...
use crate::domain::sensor_data::{init_landmark, Landmark, SensorData};
use nalgebra as na;

pub fn slam(path: &str) -> (Vec<(f32, f32, f32)>, Vec<Vec<SensorData>>, [Landmark; 6]) {
    let (mut hat_xs, sensor_data, us) = file::pose_read(path);
    let mut delta_xs;

    let landmark_list = constraint(&sensor_data);
    /* slamの最適化サイクルについて、定数化要 */
    for _n in 0..3 {
        delta_xs = graph_slam(&hat_xs, &us, &landmark_list);
    ...
    for (id, landmark) in landmark_list.iter().enumerate() {
        ...
        for i in 0..landmark_list.len() {
            let mut edge = MapEdge::new();
            let t = landmark[i].0;
            let z = landmark[i].1.data.clone();
